...
On an unmarked return, `bin/fm-afk-return.sh` owns ordered shutdown, durable catch-up evidence, and the fail-closed gate, including its single read-only report pass-through; its header owns the exact contract.
