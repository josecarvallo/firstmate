...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, a secondmate whose own status stream goes silent past `FM_DECLARED_WORK_SILENCE_SECS` after declaring work, and heartbeat backstop hits.
...
That mate also never produces a parent-side `state/<id>.turn-ended` at all, so the ordinary bare-turn-ended absorb rule has no secondmate case to apply to; the exclusion that forces this, and what supervision reads instead, are stated below.
A secondmate's idle endpoint is healthy by design, so the stale loop exempts it, and that exemption is unchanged; what the exemption used to hide is an unresumed declaration.
A `working:` line states an intention, never proof the work was launched, so a mate that writes its plan and ends its turn leaves declared work nothing will resume: the stale loop skips the healthy idle endpoint, and the heartbeat backstop correctly absorbs a status that is neither captain-relevant nor unpresented.
The same poll therefore checks every secondmate for declared work that no later event superseded while that mate's own status stream stayed silent past `FM_DECLARED_WORK_SILENCE_SECS`, and surfaces the combination as its own `check: secondmate-stalled <id>` reason rather than as a stale pane.
The measured signal is the status log, not the endpoint, and that choice is forced rather than preferred: a secondmate can never carry parent-side turn-end or busy wiring, because `bin/fm-spawn.sh` excludes `kind=secondmate` from all of it so the parent cannot land on the turn boundary that keeps the mate's own primary supervision alive (see [turnend-guard.md](turnend-guard.md)), and reading its rendered pane instead was rejected as a fragile surface and a false-alarm source.
Status files are append-only, so the log's own age is the time since its last event - a durable record this host already holds, needing no launch wiring, and reaching a remote mate through the same reading because its escalations are ingested into this same state directory.
For a remote mate that reading measures the parent-side copy the ingest writes, so a relay that stopped delivering presents the same frozen record as a mate that stopped; the wake names that cause alongside the others, and distinguishing them would take transport health monitoring, which is deliberately out of scope for this change.
`fm-classify-lib.sh` owns the decision and its vocabulary (`status_declares_work`, `status_declared_work_stalled`) in the one library both supervisors source, and away mode wraps this same watcher, so the check runs unchanged in both modes and its wake reaches the daemon's routing through that single supervision cycle; the last status line the poll already read decides first, so an ordinary fleet pays essentially nothing per poll.
A declared `paused:` wait and a terminal last event each keep their existing meaning and never become a stall.
The wake is reconciliation rather than an alarm, because silence is not proof: the mate may be doing long legitimate work with nothing supervisor-actionable to report, may have finished a phase without closing its record, may genuinely be stopped, or - remotely - may be reporting normally behind a relay that stopped delivering its events here, and from outside those are indistinguishable.
That is also why the threshold is far more generous than the wedge threshold - this repo's own status contract makes appends sparse events rather than routine progress - and why the reason asks for a cheap check before acting.
It surfaces exactly once per declared-work episode, latched until a later status event ends the episode, because the durable queue is acknowledged only after the handling turn and so one wake cannot be lost.
The latch records the status log's own size:mtime signature, the same episode boundary the signal scan uses, so the event that ends an episode ends it even when it lands while no watcher is running - watcher downtime can never leave the detector permanently latched against the one mate it exists to catch.
That single bounded wake is what the last-event rule honestly supports: it narrows, rather than eliminates, the unsuperseded-start-event false signal the registered-secondmate current-state section below records, and whichever reading holds, the mate's own stream disagrees with reality, so the wake states every reading it cannot rule out and suppression is deliberately not built.
...
The explicit resolution is written by the actor that answers, not the busy worker: `fm-send`'s `--resolve-key` appends the closing `resolved` line to every live copy of that key at answer time, which covers crewmates, local secondmates, and remote secondmates identically because a remote mate's escalations reach this home's copy through the parent-replies ingest and only the answer message itself crosses the transport.
A secondmate home holds a second live copy whenever it relayed a worker's decision onto its parent escalation channel, so the same answer closes that copy too and a decision always closes in the channel where it opened; `bin/fm-parent-channel-lib.sh` owns which channel that is for a given home, and the `fm-send.sh` header owns the propagation and refusal contract.
...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill starts it through `bin/fm-afk-launch.sh`, whose detached terminal runs the launcher-only foreground entry `bin/fm-afk-start.sh`, after which the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
...
`bin/fm-spawn.sh` never arms that wiring for a `kind=secondmate` task whatever its harness, so such a task has no busy record and no parent-side turn-end signal to classify; event-driven supervision above owns why, and what it reads instead.
